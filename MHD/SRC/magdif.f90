module magdif
  ...
  real(dp), parameter :: echarge = 1.602176d-12
...
contains
  ...
  subroutine init
    ...
    open(1, file = 'q.out')
    do k = 2, nr_max - 1
       write (1,*) (psi(k+1) + psi(k)) * 0.5d0, q(k), dqdpsi(k), (q(k+1) - q(k)) / ((psi(k+1) - psi(k-1)) * 0.5d0)
    end do
    close(1)
    ...
    do k = 1, ntri
       ...
       if (abs((sum(Bnflux(k,:)) + &
            (0d0,1d0) * n * Bnp(k) * mesh_element(k)%det_3 * 0.5d0) / sum(Bnflux(k,:))) > 1d-10) then
  ...
  subroutine test_triangle_strip
    ...
    do k = npmin, npmax
       a(k - npmin + 1) = 1d0 / hypot(mesh_point(k)%rcoord, mesh_point(k)%zcoord)
    end do
...
!qn(kp-1) = (oldknot%b_mod * rold + curknot%b_mod * r) * 0.5d0 * &
!             sum(mesh_element_rmp(common_tri(:))%bnorm_vac) * 0.5d0

        a(kp-1) = ((Br + Brold) * 0.5d0 * (r - rold) + &
             (Bz + Bzold) * 0.5d0 * (z - zold)) / ((r - rold) ** 2 + (z - zold) ** 2)

!a(kp-1) = (r + rold) * 0.5d0 * 1d3 / hypot(r - rold, z - zold)
!a(kp-1) = (Br * (r - rold) + Bz * (z - zold)) / ((r - rold) ** 2 + (z - zold) ** 2)
!a(kp-1) = 1e3*z + (0d0,1d0) * 1d3 * r
        b(kp-1) = (0d0,1d0) * n * (Bp / r + Bpold / rold) * 0.5d0
!b(kp-1) = (0d0,1d0) * n * (Bp + Bpold) * 0.5d0
!b(kp-1) = (0d0,1d0) * n * 1d0 * 0.5d0
      end do ! kp
       ...
       call common_triangles(oldknot, curknot, common_tri)
       x(nt_core) = c(nt_core) * (oldknot%b_mod + curknot%b_mod) * 0.5d0 * &
            sum(mesh_element_rmp(common_tri(:))%bnorm_vac) * 0.5d0

!qn(nt_core) = (oldknot%b_mod * rold + curknot%b_mod * r) 0.5d0 &
!             sum(mesh_element_rmp(common_tri(:))%bnorm_vac) * 0.5d0

       a(nt_core) = ((Br + Brold) * 0.5d0 * (r - rold) + &
            (Bz + Bzold) * 0.5d0 * (z - zold)) / ((r - rold) ** 2 + (z - zold) ** 2)
!a(nt_core) = (r + rold) * 0.5d0 * 1d3 / hypot(r - rold, z - zold)
!a(nt_core) = (Br * (r - rold) + Bz * (z - zold)) / ((r - rold) ** 2 + (z - zold) ** 2)
!a(nt_core) = 1e3 * z + (0d0,1d0) * 1d3 * r
       b(nt_core) = (0d0,1d0) * n * (Bp / r + Bpold / rold) * 0.5d0
!b(nt_core) = (0d0,1d0) * n * (Bp + Bpold) * 0.5d0
!b(nt_core) = (0d0,1d0) * n * 1d0 * 0.5d0
  ...
  subroutine test_current
    ...
    do kt = 1, nt_core
       ...
       call field(r, p, z, Br, Bp, Bz, dBrdR, dBrdp, dBrdZ, &
            dBpdR, dBpdp, dBpdZ, dBzdR, dBzdp, dBzdZ)
       b(kt) = -(0d0,1d0) * n * mesh_element(kt)%det_3 * 0.5d0 * Bp / dpsi
...
!write (1,*) kt, real(b(kt)), aimag(b(kt)), real(x(kt)), aimag(x(kt))
    ...
    do kl = 2, nr_max
       ...
       do kt = 1, 2 * nt_core
          ...
          call field(r, p, z, Br, Bp, Bz, dBrdR, dBrdp, dBrdZ, &
               dBpdR, dBpdp, dBpdZ, dBzdR, dBzdp, dBzdZ)
          b(kt) = -(0d0,1d0) * n*elem%det_3 *0.5d0 * Bp / dpsi
          x(kt) = -curr(kt, mod(elem%knot_h,3)+1) + get_source(kt,kl)
       end do !kt
    ...
    do kt = 1, ntri
       ...
       jpar = (Br * jr + Bz * jz) / hypot(Br, Bz)
       jperp = (Bz * jr - Br * jz) / hypot(Br, Bz)
