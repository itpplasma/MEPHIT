...
########print(routine_name)
        ...
        for arg in args:
############print(type(arg))
            if (ka==0 and isinstance(arg, object) and 
                arg.__class__.__name__ == 'PyCSimpleType'):
    ...
    def __var_fortran(self, function, *args):
        ...
        if(len(args) == 1):
            return dtype.in_dll(self.library, '__{}_MOD_{}'.format(self.name, function))
        ...
        return np.ctypeslib.as_array(POINTER(dtype).in_dll(
                self.library, '__{}_MOD_{}'.format(self.name, function)), shape)[:,0]
