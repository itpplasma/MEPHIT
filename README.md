...
- [FortFEM](https://github.com/lazy-fortran/fortfem) for the native
  finite-element backend
- [Triangle](https://www.cs.cmu.edu/~quake/triangle.html) and
  [FreeFem++](https://github.com/FreeFem/FreeFem-sources) only for the legacy
  finite-element backend
...
- `--debug` starts a [GDB](https://www.gnu.org/software/gdb) session, using
  [a script](scripts/mephit.gdb) that stops at the main Fortran subroutine,
...
- legacy-backend runs also produce `core_plasma*.msh`, `outer*.msh`, and
  `maxwell*.msh` for FreeFem++, `edgemap*.dat` for edge-degree-of-freedom
  transfer, and `fglut_dump*` graphics viewable with `ffglut`.
