module magdif_conf
  ...
  type :: magdif_config
     ...
     !> Formatted output data file for magnetic field perturbation (vacuum). Defaults to
     !> `Bn_vac.dat`.
     !>
     !> This file is generated by magdif::init().
     character(len = 1024) :: Bn_vacout_file = 'Bn_vac.dat'

     !> Formatted input data file for magnetic field perturbation (vacuum). Defaults to
     !> `Bnflux.dat`
     !>
     !> This file is generated by `magdif.bash` and is read in by magdif::init() via
     !> magdif::read_bn().
     character(len = 1024) :: Bn_vac_file = 'Bn_flux.dat'

     !> Formatted data file for accumulated magnetic field perturbation. Defaults to
     !> `Bn.dat`.
     !>
     ...
     !> #bn_vac_file.
     character(len = 1024) :: Bn_file = 'Bn.dat'

     !> Formatted output data file for magnetic field perturbation (plasma response).
     !> Defaults to `Bn_diff.dat`.
     !>
     !> This file is generated in every iteration by magdif::compute_bn() and read in
     !> afterwards via magdif::read_bn().
     character(len = 1024) :: Bn_diff_file = 'Bn_diff.dat'

     !> Formatted output data file containing flux variables. Defaults to `fluxvar.dat`.
     !>
     !> This file is generated by magdif::magdif_init(). Its columns contain, in that order,
     !> radius \f$ r \f$, flux \f$ \psi \f$, safety factor \$f q \f$, pressure
     !> \f$ p_{0} \f$, and the pressure derivative \f$ p_{0}'(\psi) \f$.
     character(len = 1024) :: fluxvar_file = 'fluxvar.dat'

     !> Output data file for calculated magdif::j0phi. Defaults to `j0phi.dat`.
     character(len = 1024) :: j0phi_file = 'j0phi.dat'

     !> Output data file for pressure perturbation. Defaults to `presn.dat`.
     character(len = 1024) :: presn_file = 'presn.dat'

     !> Output data file for current perturbation. Defaults to `currn.dat`.
     character(len = 1024) :: currn_file = 'currn.dat'

     !> Output data file for eigenvectors in magdif::magdif_iterated(). Defaults to
     !> `eigvec.dat`.
     character(len = 1024) :: eigvec_file = 'eigvec.dat'
