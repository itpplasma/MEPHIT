...
\end{align*}
so if $B_{0\ph}=\mathrm{const.}$ we are allowed to use any value
$B_{n}^{\psi}=C(\psi)/R^{2}$, where $C(\psi)$ is a flux function,
to generate a non-resonant field. The other components of $\delta\v B$
are not relevant for the resonance condition and just need to fulfill
divergence-freeness.
