...
which is the negative disk poloidal flux. Since $\v B_{\mathrm{pol}}$
can be pointed in either orientation, we add a factor $\sigma_{\mathrm{pol}}=+1$
for counterclockwise and $\sigma_{\mathrm{pol}}=-1$ for clockwise
direction of $\v B_{\mathrm{pol}}$ in the poloidal plane. Thus $\psi_{\mathrm{pol}}(\rho)=\sigma_{\mathrm{pol}}\rho$
and $\psi_{\mathrm{pol}}^{\prime}(\rho)=\sigma_{\mathrm{pol}}$. The
safety factor is then given by
\begin{align*}
