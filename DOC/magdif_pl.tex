...
\end{gather}
Here, a compatibility condition needs to be fulfilled -- the flux through one flux surface shell, so the sum over all triangles $i$ must vanish with
\begin{gather}
  \sum_{i} (\Psi_{\phi}^{(i)} + \Psi_{\fs}^{(i)}) = 0.
\end{gather}
