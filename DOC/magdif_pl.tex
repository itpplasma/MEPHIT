...
\end{gather*}
where $C(\psi)$ is a flux function. The other components of $\delta \vec{B}$ are not relevant for the resonance condition and just need to fulfill divergence-freeness. For any given $B_{n}^{\phi}$ we proceed as in \cref{sec:compute_currn} and find fluxes through triangle edges:
\begin{gather}
...
\end{gather}
The term through edge \fs\ orthogonal to $n_{\fs} \parallel \nabla \psi$ is
\begin{gather}
  -\Psi_{\fs} \approx \mp R_{\fs} l_{\fs} \frac{B_{n}^{\psi}}{\lvert \nabla \psi \rvert} \label{eq:Psi_f}
\end{gather}
with sign depending on edge orientation. The second term yields
\begin{gather*}
  -\Psi_{\phi} = -\im n \int_{\Omega} \diff R \, \diff Z \, R B_{n}^{\phi} \approx - \im n S_{\Omega} \, R_{k} B_{n k}^{\phi}.
\end{gather*}
...
and
\begin{gather}
  q^{k} = \mp R_{\fs} l_{\fs} \frac{B_{n}^{\psi}}{\lvert \nabla \psi \rvert} - \im n S_{\Omega} R_{k} B_{n k}^{\phi}.
\end{gather}
