...
With the usual toroidal flux angle $\zeta=\pi/2-\ph$ (dHaeseleer),
the toroidal flux $\psi_{\text{tor}}$ is given by
\begin{align}
\psi_{\text{tor}} & =\int\d V\,\boldsymbol{B}\cdot\nabla\zeta=-\int\d V\,\boldsymbol{B}\cdot\nabla\ph\nonumber \\
 & =-2\pi\int\d R\d Z\,RB^{\ph}=-2\pi\int\d R\d ZB_{(\ph)}.
\end{align}
As a radial flux variable $\rho$ we use the (ribbon) poloidal flux
$\psi_{\mathrm{pol}}$ which is the negative disk poloidal flux,
\[
\rho=\psi_{\mathrm{pol}}=-\psi.
\]
Thus the safety factor is given by
\begin{align*}
q & =\frac{\psi_{\mathrm{tor}}^{\prime}(\rho)}{\psi_{\mathrm{pol}}^{\prime}(\rho)}=\frac{\d\psi_{\mathrm{tor}}/\d\psi_{\mathrm{pol}}}{1}=-\frac{\d\psi_{\mathrm{tor}}}{\d\psi}\\
 & =+2\pi\frac{\d}{\d\psi}\int\d R\d ZB_{(\ph)}.
\end{align*}
...
difference in $\psi$:
\[
q\approx\frac{2\pi}{\Delta\psi}\sum_{\triangle}B_{(\ph)}S_{\triangle},
\]
...
For derivation we use symmetry flux coordinates $(\rho,\vartheta,\zeta)=(-\psi,\vartheta,\frac{\pi}{2}-\varphi)$
with Jacobian
\begin{align}
...
is
\begin{align*}
-\Psi_{3} & \approx\mp R_{3}l_{3}\frac{B_{n}^{\psi}}{|\nabla\psi|}
\end{align*}
