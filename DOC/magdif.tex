...
\begin{align}
  \hat{K} \left ( \delta \vec{B}_{\text{v}} + \delta \vec{B}_{\text{p}} \right ) &= \delta \vec{B}_{\text{p}}, \label{eq:K_fixed-point} \\
...
\end{gather}
This way a consistent solution for $\delta \vec{B}_{\text{p}}$ can be computed from $\delta \vec{B}_{\text{v}}$ by repeated application of $\hat{K}$, given explicitly by the infinite series
\begin{gather}
  \delta \vec{B}_{\text{p}} = \left ( \hat{1} + \hat{K} + \hat{K}^{2} + \dotsb \right ) \hat{K} \delta \vec{B}_{\text{v}} = \sum_{k = 1}^{\infty} \hat{K}^{k} \delta \vec{B}_{\text{v}} = \sum_{k = 1}^{\infty} \delta \vec{B}_{\text{p}}^{(k)}. \label{eq:K_series}
\end{gather}
In \cref{eq:K_series} each term is given by the recurrence relation
\begin{gather}
  \delta \vec{B}_{\text{p}}^{(k+1)} = \hat{K} \delta \vec{B}_{\text{p}}^{(k)}.
\end{gather}
Adding the vacuum field as the initial value,
\begin{gather}
  \delta \vec{B}_{\text{p}}^{(0)} = \delta \vec{B}_{\text{v}}
\end{gather}
the series' terms are accumulated for the self-consistent solution:
\begin{gather}
  \delta \vec{B} = \sum_{k = 0}^{\infty} \delta \vec{B}_{\text{p}}^{(k)}.
\end{gather}
Alternatively, \cref{eq:K_fixed-point} can be expanded,
\begin{gather}
  \delta \vec{B}_{\text{p}} = \hat{K} \left ( \delta \vec{B}_{\text{v}} + \delta \vec{B}_{\text{p}} \right ) = \hat{K} \left ( \delta \vec{B}_{\text{v}} + \hat{K} \left ( \delta \vec{B}_{\text{v}} + \delta \vec{B}_{\text{p}} \right ) \right ) = \dotsb,
\end{gather}
yielding a fixed-point iteration for $\delta \vec{B}_{\text{p}}$:
\begin{gather}
  \delta \vec{B}_{\text{p}}^{[k+1]} = \hat{K} \left ( \delta \vec{B}_{\text{v}} + \delta \vec{B}_{\text{p}}^{[k]} \right ).
\end{gather}
Compared to the previous approach, this one is cumulative, i.e. it immediately produces the next approximation of the plasma perturbation. In other words, it corresponds to the sequence of partial sums of the previous infinite series:
\begin{gather}
  \delta \vec{B}_{\text{p}}^{[n]} = \sum_{k = 1}^{n} \delta \vec{B}_{\text{v}}^{(k)}. \label{eq:K_partial_sum}
\end{gather}
For this to be consistent, the initial value is taken to be zero,
\begin{gather}
  \delta \vec{B}_{\text{p}}^{[0]} = \vec{0},
\end{gather}
which makes sense since it represents the empty sum. As a consequence, even though the vacuum field is added in every iteration step, the final result does not contain the vacuum field. In the previous approach, the sumamtion can simply be extended to $k = 0$; here, it has to be added explicitly after the last iteration. For illustration, both approaches are compared side-by-side in \cref{tab:comp_iter}.
\begin{table}[htb]
  \begin{align*}
    \text{iteration step:} && \delta \vec{B}_{\text{p}}^{(k)} &= \hat{K} \delta \vec{B}_{\text{p}}^{(k-1)} & \delta \vec{B}_{\text{p}}^{[k]} &= \hat{K} \left ( \delta \vec{B}_{\text{p}}^{[k-1]} + \delta \vec{B}_{\text{v}} \right ), \\
    \text{initial value:} && \delta \vec{B}_{\text{p}}^{(0)} &= \delta \vec{B}_{\text{v}} & \delta \vec{B}_{\text{p}}^{[0]} &= \vec{0}, \\
    \text{step 1:} && \delta \vec{B}_{\text{p}}^{(1)} &= \hat{K} \delta \vec{B}_{\text{p}}^{(0)} = \hat{K} \delta \vec{B}_{\text{v}} & \delta \vec{B}_{\text{p}}^{[1]} &= \hat{K} \left ( \delta \vec{B}_{\text{p}}^{[0]} + \delta \vec{B}_{\text{v}} \right ) = \hat{K} \delta \vec{B}_{\text{v}}, \\
    \text{step 2:} && \delta \vec{B}_{\text{p}}^{(2)} &= \hat{K} \delta \vec{B}_{\text{p}}^{(1)} = \hat{K}^{2} \delta \vec{B}_{\text{v}} & \delta \vec{B}_{\text{p}}^{[2]} &= \hat{K} \left ( \delta \vec{B}_{\text{p}}^{[1]} + \delta \vec{B}_{\text{v}} \right ) = \left ( \hat{K}^{2} + \hat{K} \right ) \delta \vec{B}_{\text{v}}, \\
    \text{explicit form:} && \delta \vec{B}_{\text{p}}^{(n)} &= \hat{K}^{n} \delta \vec{B}_{\text{v}} & \delta \vec{B}_{\text{p}}^{[n]} &= \sum_{k = 1}^{n} \hat{K}^{k} \delta \vec{B}_{\text{v}}, \\
    \text{full plasma response:} && \delta \vec{B}_{\text{p}} &= \sum_{k = 1}^{\infty} \hat{K}^{n} \delta \vec{B}_{\text{v}} & \delta \vec{B}_{\text{p}} &= \delta \vec{B}_{\text{p}}^{[\infty]}, \\
    \text{full perturbation:} && \delta \vec{B} &= \sum_{k = 0}^{\infty} \hat{K}^{n} \delta \vec{B}_{\text{v}} & \delta \vec{B} &= \delta \vec{B}_{\text{p}}^{[\infty]} + \delta \vec{B}_{\text{v}}
  \end{align*}
  \caption{Comparison of iteration with series (non-cumulative) and sequence (cumulative)}
  \label{tab:comp_iter}
\end{table}
For Arnoldi iterations, the cumulative approach is more convenient. To reproduce the intermediate summands, we use \cref{eq:K_partial_sum} and arrive at
\begin{gather}
  \delta \vec{B}_{\text{p}}^{(k)} = \delta \vec{B}_{\text{p}}^{[k]} - \delta \vec{B}_{\text{p}}^{[k-1]}.
\end{gather}

\subsection{Enhanced convergence with Arnoldi iterations}
\label{sec:Arnoldi}

Assuming the linear operator $\hat{K}$ is non-singular, we get an eigendecomposition
\begin{gather}
...
\end{gather}
and $\hat{V}$ contains the corresponding eigenvectors as its columns,
\begin{gather}
  \hat{V} = \left ( \vec{v}_{1}, \vec{v}_{2}, \dotsc, \vec{v}_{N} \right ),
\end{gather}
and $\hat{V}^{-1}$ is the inverse of $\hat{V}$. Repeated application of $\hat{K}$ then simplifies to
\begin{gather}
  \hat{K}^{k} = \underbrace{\left ( \hat{V} \hat{\Lambda} \hat{V}^{-1} \right ) \left ( \hat{V} \hat{\Lambda} \hat{V}^{-1} \right ) \dotsb \left ( \hat{V} \hat{\Lambda} \hat{V}^{-1} \right )}_{k} = \hat{V} \hat{\Lambda}^{k} \hat{V}^{-1}.
\end{gather}
