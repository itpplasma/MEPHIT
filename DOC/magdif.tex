...
\end{gather}
where $\vec{S}$ is the disc that at a point of evaluation $(R_{S}, Z_{S})$ is given by $R \leq R_{S}, Z = Z_{S}$. The orientation of $\vec{S}$ fixes the sign\footnote{Since $\vec{B}_{0}$ is not defined outside the plasma volume and only $\nabla \psi$ enters calculations, $\psi$ is defined up to a constant and thus could be shifted to change sign. However, the sign of $\nabla \psi$ is fixed.} of $\psi$. Here, $\psi$ is expected to increase towards the magnetic axis -- $\psi_{\text{min}}$ is located at the outermost flux surface and $\psi_{\text{max}}$ is located at the magnetic axis. This leads to the following contravariant components for $\vec{B}_{0}^{\pol}$ in cylindrical coordinates:
\begin{align}
...
[\textsc{Raviart}--\textsc{Thomas} elements: DOFs are fluxes across edges, interpolation, \ldots]
\begin{gather*}
...
In the formulae derived in \cref{sec:linmhd}, terms of the form $\vec{v} \cdot \nabla \psi$ appear, where $\vec{v}$ is an arbitrary vector, usually the vector of a triangle edge or some field quantity. In the latter case, this reduces to the projection on the normal vector of an edge, since perturbed vector fields are represented by \textsc{Raviart}-\textsc{Thomas} elements. [\ldots]

The relation of the degrees of freedom to the contravariant $\psi$ component of the magnetic perturbation on edge \fs\ is derived here as an example, as it is used for computations in \cref{sec:compute_presn,sec:nonres}. The general expression on a given triangle is given by
\begin{gather}
  B_{n}^{\psi} (\Gamma_{\fs}) = \vec{B}_{n} (\Gamma_{\fs}) \cdot \nabla \psi (\Gamma_{\fs}) = \vec{B}_{n} (\Gamma_{\fs}) \cdot \hat{\vec{n}}_{\fs} \pd[\psi]{n_{\fs}} (\Gamma_{\fs}).
\end{gather}
...
\begin{align}
  ...
  &= \vec{J}_{n} \cdot \left( (\vec{l} \cdot \grad \psi) \grad \phi + R B_{0}^{\phi} \vec{n} \right) \nonumber \\
  &= (\vec{l} \cdot \grad \psi) J_{n}^{\phi} + R B_{0}^{\phi} \vec{J}_{n}^{\pol} \cdot \vec{n}.
\end{align}
Further,
\begin{gather}
...
Finally,
\begin{gather}
  \vec{l} \cdot (\vec{J}_{n} \times \vec{B}_{0}) = R B_{0}^{\phi} \vec{J}_{n}^{\pol} \cdot \vec{n} - R J_{n}^{\phi} \vec{B}_{0}^{\pol} \cdot \vec{n}.
\end{gather}
...
\begin{align}
  ...
  &= \frac{c p_{0}' (\psi) - J_{0}^{\phi}}{B_{0 \phi}}  \grad \psi.
\end{align}
Also
\begin{align}
...
Finally
\begin{gather}
  (\vec{l} \cdot \grad \psi) J_{n}^{\phi} + R B_{0}^{\phi} \vec{J}_{n}^{\pol} \cdot \vec{n} = c \vec{l} \cdot \grad p_{n} - \frac{B_{n \phi}}{B_{0 \phi}} \left( c \grad p_{0}' (\psi) - J_{0}^{\phi} \right) \vec{l} \cdot \grad \psi + R J_{0}^{\phi} \vec{B}_{n}^{\pol} \cdot \vec{n} \label{eq:jnphi-jnpol}
\end{gather}
