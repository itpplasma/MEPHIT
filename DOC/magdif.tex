...
\begin{gather}
  B_{n}^{\psi} = \frac{\Psi_{\fs}}{R l} \frac{\psi^{(+1)} - \psi^{(-1)}}{a_{\fs} (\Omega^{(+1)}) + a_{\fs} (\Omega^{(-1)})} = \frac{\Psi_{\fs}}{R \cancel{l_{\fs}}} \frac{\psi^{(+1)} - \psi^{(-1)}}{S_{\Omega^{(+1)}} + S_{\Omega^{(-1)}}} \cancel{l_{\fs}}.
\end{gather}
[\ldots]
