...
with index $k\leq m$. This means that $\hat{\bar{K}}\v v_{k}=0$,
\begin{align}
0=\hat{\bar{K}}\v v_{k} & =\hat{M}\v v_{k}+(\hat{I}-\hat{M})\lambda_{k}\v v_{k}\nonumber \\
 & =\lambda_{k}\v v_{k}+\hat{M}(1-\lambda_{k})\v v_{k}.
\end{align}
Combining $\v v_{k}$ inside $\hat{V}_{m}$ equivalently yields
\[
\hat{\bar{K}}\hat{V}_{m}=\hat{V}_{m}\hat{\Lambda}_{m}+\hat{M}\hat{V}_{m}(\hat{I}_{m}-\hat{\Lambda}_{m})=0
\]
so
\begin{equation}
\hat{M}\hat{V}_{m}(\hat{\Lambda}_{m}-\hat{I}_{m})=\hat{V}_{m}\hat{\Lambda}_{m}.
\end{equation}
Right-Multiplying with the inverse of $(\hat{\Lambda}_{m}-\hat{I}_{m})$
yields
\begin{equation}
\hat{M}\hat{V}_{m}=\hat{V}_{m}\hat{\Lambda}_{m}(\hat{\Lambda}_{m}-\hat{I}_{m})^{-1}.
\end{equation}
...
\begin{equation}
\hat{M}\hat{V}_{m}=\hat{V}_{m}\hat{\Lambda}_{m}(\hat{\Lambda}_{m}-\hat{I}_{m})^{-1}(\hat{V}_{m}^{\star}\hat{V}_{m})^{-1}\hat{V}_{m}^{\star}\hat{V}.
\end{equation}
...
by
\begin{equation}
\hat{M}=\hat{V}_{m}\hat{\Lambda}_{m}(\hat{\Lambda}_{m}-\hat{I}_{m})^{-1}(\hat{V}_{m}^{\star}\hat{V}_{m})^{-1}\hat{V}_{m}^{\star}.
\end{equation}
...
by solving the $m$-dimensional linear system
\begin{equation}
...
the preconditioner $\hat{\Pi}^{-1}=\hat{I}-\hat{M}$ as
\begin{equation}
...
\]
wherehaving the same solution as the original equation, but with stable
