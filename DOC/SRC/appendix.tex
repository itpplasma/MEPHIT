...
In general, variables are printed in italic shape and constants in roman (upright) shape. Scalars are printed in normal weight, vectors are indicated by bold weight, matrices and operators by a hat, and tensors by sans-serif font. We only list symbols that appear in more than one chapter, or when they are used differently in different chapters. Symbols from \cref{tab:symbols} may appear decorated as listed in \cref{tab:decorations}.
