...
\end{align}
To solve this equation on one flux surface, we use a lowest-order finite difference method. Nodes are indexed by superscript $(k)$ and $\vec{r}^{(k)}$ is the position of node $(k)$, whereas $\vec{l}_{\fs}^{(k)} = \vec{r}^{(k+1)} - \vec{r}^{(k)}$ is the counter-clockwise vector between nodes on edge $\Gamma_{\fs}^{(k)}$. $\grad p_{n}$ is approximated at the midpoint of edge $\Gamma_{\fs}^{(k)}$ as finite difference\footnote{The gradient reduces to a directional derivative along the edge $\vec{l}_{\fs}$ because of the scalar product with $\vec{B}_{0}^{\pol}$, which approximately points in this direction. This relationship could be written as $B_{0}^{\theta} p_{n}'(\theta)$ in flux symmetry coordinates and allows for further simplifications.} of $p_{n}$ at nodes $(k)$ and $(k+1)$. $p_{n}$ is accordingly approximated at the midpoint as the arithmetic mean of the values at these nodes. With a shorthand $p_{n}^{(k)} = p_{n} (\vec{r}^{(k)})$ for the degrees of freedom we get
\begin{gather}
...
\begin{align}
  ...
  s_{k} &= -\td[p_{0}]{\psi} (\Gamma_{\fs}^{(k)}) B_{n}^{\psi} (\Gamma_{\fs}^{(k)}) \overset{\eqref{eq:Bnpsi}}{=} -\td[p_{0}]{\psi} (\Gamma_{\fs}^{(k)}) \Psi_{\fs}^{(k)} \frac{a_{k}}{l_{\fs}^{(k)}}.
\end{align}
...
\end{gather}
On edge \fs, $\vec{B}_{0}^{\pol} \cdot \vec{n}_{\fs} = 0$ and $\grad p_{0} \cdot \vec{l}_{\fs} = 0$ (compare \cref{eq:l_grad_psi}), thus from \cref{eq:jnphi-jnpol}, no connection between $J_{n}^{\phi}$ and $\vec{J}_{n}^{\pol} \cdot \vec{n}_{\fs}$ can be made, but the latter expression can be given in terms of already known quantities:
\begin{gather}
  R \vec{J}_{n}^{\pol} \cdot \vec{n}_{\fs} = \frac{c \grad p_{n} \cdot \vec{l}_{\fs}}{B_{0}^{\phi}} + R \frac{J_{0}^{\phi}}{B_{0}^{\phi}} \vec{B}_{n}^{\pol} \cdot \vec{n}_{\fs}. \label{eq:If}
\end{gather}
...
\begin{gather}
  \oint_{\partial \Omega^{(k)}} R \vec{J}_{n}^{\pol} \cdot \hat{\vec{n}} \, \diff l = \int_{\Gamma_{\inw}^{(k)}, \Gamma_{\out}^{(k)}} R \vec{J}_{n}^{\pol} \cdot \hat{\vec{n}} \, \diff l + \int_{\Gamma_{\fs}^{(k)}} R \vec{J}_{n}^{\pol} \cdot \hat{\vec{n}} \, \diff l,
\end{gather}
