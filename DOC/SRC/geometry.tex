...
\end{gather}
Here, $\varepsilon_{ijk}$ is the \textsc{Levi}--\textsc{Cività} symbol. On occasion, parallel or perpendicular components with respect to the magnetic field $\vec{B}$ are needed:
\begin{gather}
...
\end{align}
Note that the toroidal field coils produce a magnetic field that is roughly proportional to $\frac{1}{R}$, so $B_{0 \phi}$ varies only slightly over the entire plasma volume. More specifically, the flux surface quantity
\begin{gather}
  F(\psi) = B_{0 \phi} \label{eq:F}
\end{gather}
appears in the \textsc{Grad}--\textsc{Shafranov} equation, so from a given solution, $B_{0 \phi}$ can be reconstructed.
