...
\begin{gather*}
  \vec{B}_{0} \cdot \nabla f = s
\end{gather*}
where $s$ is the source and $f$ is the unknown. We are solving for a single Fourier mode over the toroidal angle to transform it to
\begin{gather*}
...
\begin{enumerate}
\item in the equation for the perturbed pressure, $n$ should be modified in the quantity $b_{k}$ in \cref{sec:compute_presn};
\item in the equation for the current, $n$ should be modified in the matrix $K_{jk}$ in \cref{sec:compute_currn}.
\end{enumerate}
...
\begin{gather*}
  \symup{\Delta} r \frac{q'(r)}{q} \ll \epsilon \ll 1,
\end{gather*}
where $q'(r)$ is the radial derivative of the safety factor $q$.
...
\subsection{Non-resonant test field: alternative version including safety factor}
